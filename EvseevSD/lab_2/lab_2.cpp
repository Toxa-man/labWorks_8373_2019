...
void signature_test(ifstream &picture) {
	long long unsigned signa_from_file;
	long long unsigned valid_signa = 0x89504E470D0A1A0A;
...
void IHDR_test(ifstream &picture) {
	unsigned ihdr_length;
	unsigned ihdr_type;
	unsigned valid_ihdr = 0x49484452;
	picture.read((char*)& ihdr_length, 4);// space for IHDR length
	...
	picture.read((char*)& ihdr_type, 4);// space for chunk type IHDR
	...
	picture.seekg(13 + 4, ios::cur);// 13 bytes - space for chunk data, 4 bytes - space for CRC
}

void IEND_and_IDAT_test(ifstream &picture) {
	...
	cout << "IEND founded" << endl << "File parsed successfully" << endl;

}
int main() {
	ifstream picture("D:\\Semka\\png_parser\\build\\img\\wrong_IHDR.png", ios::binary);
